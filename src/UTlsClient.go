...
type UTlsRequest struct {
	...
	Port        string // 端口号，默认为80（HTTP）或443（HTTPS）
}
...
func (c *UTlsClient) connectWithIP(req *UTlsRequest) (net.Conn, error) {
	// 解析IP地址
	...
	if req.LocalIP != "" {
		...
		tcpConn, err = dialer.Dial("tcp", net.JoinHostPort(req.DomainIP, req.Port))
	} else {
		// 使用默认本地地址
		tcpConn, err = net.Dial("tcp", net.JoinHostPort(req.DomainIP, req.Port))
	}
...
func (c *UTlsClient) connectWithDomain(req *UTlsRequest) (net.Conn, error) {
	// 解析域名获取IP地址
...
func (c *UTlsClient) readHTTPResponse(conn net.Conn) (int, []byte, error) {
	...
	// 检查是否是信息性响应(1xx)，如果是则继续读取下一个响应
	for resp.StatusCode >= 100 && resp.StatusCode < 200 {
		resp, err = http.ReadResponse(reader, nil)
		if err != nil {
			return resp.StatusCode, []byte(body.String()), fmt.Errorf("读取最终HTTP响应失败: %w", err)
		}
		defer resp.Body.Close()
		...
		_, err = bufio.NewReader(resp.Body).WriteTo(body)
		if err != nil {
			return resp.StatusCode, []byte(body.String()), fmt.Errorf("读取最终响应体失败: %w", err)
		}
	}

	return resp.StatusCode, []byte(body.String()), nil
}
