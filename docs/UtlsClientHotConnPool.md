...
├─────────────────────────────────────────────────────────────┤
│  TLS会话缓存                                                │
│  └─ sessionCache: utls.ClientSessionCache (LRU缓存，1000个会话)│
├─────────────────────────────────────────────────────────────┤
│  依赖组件                                                    │
...
3. 初始化连接池
   ...
   ├─ 设置TLS指纹配置
   ├─ 创建TLS会话缓存 (sessionCache: utls.NewLRUClientSessionCache(1000))
   └─ 初始化UTlsClient用于健康检查
...
- **白名单**：
  - HTTP/2连接：TLS握手成功即加入白名单
  - HTTP/1.1连接：返回200且body长度为13字节才加入白名单
- **黑名单**：返回403的IP自动加入黑名单
...
- **sessionCache**：使用 `utls.NewLRUClientSessionCache(1000)` 创建LRU缓存
- **作用**：支持TLS会话恢复，减少TLS握手开销
- **适用场景**：新连接建立时，如果之前有缓存的会话票据，可以快速恢复TLS握手
- **性能提升**：将TLS握手时间从~200ms降至~50ms（会话恢复场景）

### 6. HTTP/2连接复用策略

- **预热策略**：HTTP/2连接只验证TLS握手，不发送HTTP请求，保持连接状态干净
- **复用机制**：连接放入池后，`UTlsClient` 会通过 `http2ClientConns` 缓存复用 `http2.ClientConn`
- **失效清理**：当连接失败时，自动从缓存中移除失效的连接

### 7. 连接池管理

- **健康池**：优先使用，包含已验证可用的连接（HTTP/2和HTTP/1.1）
- **不健康池**：备用，包含可能有问题但可尝试的连接
- **动态创建**：池为空时自动创建新连接（需要检查白名单）
- **连接持久化**：连接不会因空闲时间过期，只会在返回403或连接失效时关闭
